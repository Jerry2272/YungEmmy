<?php
/**
 * emmy functions and definitions.
 */

if ( ! function_exists( 'emmy_setup' ) ) {
	/**
	 ...
	 */
		function emmy_setup() {
			...
			* to change 'emmy' to the name of your theme in all the template files.
			*/
			...
			*/
			...
			register_nav_menus(
				array(
					'primary'        => esc_html__( 'Primary', 'emmy' ),
					'explore'         => esc_html__( 'Explore Menu', 'emmy' ),
					'contact'         => esc_html__( 'Contact Menu', 'emmy' ),
					'socials'         => esc_html__( 'Socials Menu', 'emmy' ),
					'header_action'  => esc_html__( 'Header Action', 'emmy' ),
					'invest'  => esc_html__( 'Invest Industry', 'emmy' ),
					'navigate'  => esc_html__( 'Navigate', 'emmy' ),
					'info'  => esc_html__( 'Info Menu', 'emmy' ),
				)
			);
...
add_action( 'after_setup_theme', 'emmy_setup' );
...
function emmy_content_width() {
	...
	$GLOBALS['content_width'] = apply_filters( 'emmy_content_width', 1170 );
}
 ...
 * @since 1.0.0
 */
function emmy_sidebar_widgets_init() {
	// Default Sidebar
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'emmy' ),
			'id'            => 'default-sidebar',
			'description'   => esc_html__( 'Add widgets here.', 'emmy' ),
...
add_action( 'widgets_init', 'emmy_sidebar_widgets_init' );

/**
 * Enqueue public scripts and styles.
 */
function emmy_public_scripts() {
    // Enqueue default styles
    wp_enqueue_style('default', get_template_directory_uri() . '/assets/css/default.css', array(), wp_rand(), 'all');

    // Enqueue Google Fonts
    wp_enqueue_style('google-fonts', 'https://fonts.googleapis.com/css2?family=Noto+Sans:ital,wght@0,100..900;1,100..900&display=swap', array(), null, 'all');
    wp_enqueue_style('tailwind', 'https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css', array(), null, 'all');

    // Enqueue custom stylesheet
    wp_enqueue_style('custom-style', get_stylesheet_uri(), array(), wp_rand(), 'all');

    // Enqueue Bootstrap JavaScript
	// wp_enqueue_script('emailjs', 'https://cdn.jsdelivr.net/npm/@emailjs/browser@4/dist/email.min.js', array(), null, 'all');
    // wp_enqueue_script('booking', get_template_directory_uri() . '/booking.js', array('jquery'), wp_rand(), true);
}

add_action( 'wp_enqueue_scripts', 'emmy_public_scripts' );

/**
 * Enqueue admin scripts and styles.
 */

// function emmy_admin_scripts() {
// 	// Admin styles.
// 	wp_enqueue_style( 'admin-style', get_template_directory_uri() . '/assets/css/admin-style.css', array(), wp_rand(), 'all' );
// }
// add_action( 'admin_enqueue_scripts', 'emmy_admin_scripts' );
